...
import YTDSetting from "../../components/YTDSetting.vue";
import { StoreSchema, TrayIconStyle } from "~shared/store/schema";
import { AuthToken } from "~shared/integrations/companion-server/types";
import logo from "~assets/icons/ytd.png";

declare const YTD_GIT_COMMIT_HASH: string;
declare const YTD_GIT_BRANCH: string;

const ytdCommitHash = YTD_GIT_COMMIT_HASH.substring(0, 7);
const ytdBranch = YTD_GIT_BRANCH;

const isDarwin = window.ytd.isDarwin;
const isLinux = window.ytd.isLinux;

const currentTab = ref(1);
const requiresRestart = ref(false);
const checkingForUpdate = ref(false);
const ytdVersion = await window.ytd.getAppVersion();
const updateAvailable = ref(await window.ytd.isAppUpdateAvailable());
const updateNotAvailable = ref(false);
const updateDownloaded = ref(await window.ytd.isAppUpdateDownloaded());

const store = window.ytd.store;
const memoryStore = window.ytd.memoryStore;
const safeStorage = window.ytd.safeStorage;
...
const adBlocking: StoreSchema["adBlocking"] = await store.get("adBlocking");
...
const adBlockingEnabled = ref<boolean>(adBlocking.enabled);
...
store.onDidAnyChange(async newState => {
  ...
  adBlockingEnabled.value = newState.adBlocking.enabled;
...
async function settingsChanged() {
  ...
  store.set("adBlocking.enabled", adBlockingEnabled.value);
...
async function settingChangedFile(event: Event) {
  ...
  store.set(setting, target.files.length > 0 ? window.ytd.getTrueFilePath(target.files[0]) : null);
...
function restartApplication() {
  window.ytd.restartApplication();
}

function restartApplicationForUpdate() {
  window.ytd.restartApplicationForUpdate();
}

function checkForUpdates() {
  window.ytd.checkForUpdates();
...
window.ytd.handleCheckingForUpdate(() => {
  checkingForUpdate.value = true;
});

window.ytd.handleUpdateAvailable(() => {
  checkingForUpdate.value = false;
  updateAvailable.value = true;
  updateNotAvailable.value = false;
});

window.ytd.handleUpdateNotAvailable(() => {
  checkingForUpdate.value = false;
  updateNotAvailable.value = true;
  updateAvailable.value = false;
});

window.ytd.handleUpdateDownloaded(() => {
  checkingForUpdate.value = false;
  updateNotAvailable.value = false;
  updateAvailable.value = false;
    ...
    <div class="content-container">
      ...
      <div class="content">
        ...
        <div v-if="currentTab === 2" class="appearance-tab">
          <YTDSetting v-model="alwaysShowVolumeSlider" type="checkbox" name="Always show volume slider" @change="settingsChanged" />
          <YTDSetting v-model="customCSSEnabled" type="checkbox" name="Custom CSS" @change="settingsChanged" />
          <YTDSetting
          ...
          />
          <YTDSetting v-model="zoom" type="range" max="300" min="30" step="10" name="Zoom" @change="settingsChanged" />
          <YTDSetting
            v-if="isLinux"
        ...
        <div v-if="currentTab === 3" class="playback-tab">
          <YTDSetting v-model="continueWhereYouLeftOff" name="Continue where you left off" type="checkbox" @change="settingsChanged" />
          <YTDSetting
            ...
            @change="settingsChanged"
          />
          <YTDSetting v-model="progressInTaskbar" type="checkbox" name="Show track progress on taskbar" @change="settingsChanged" />
          <YTDSetting v-model="enableSpeakerFill" type="checkbox" restart-required name="Enable speaker fill" @change="settingChangedRequiresRestart" />
          <YTDSetting v-model="ratioVolume" type="checkbox" name="Ratio volume" @change="settingsChanged" />
        </div>
        ...
        <div v-if="currentTab === 4" class="integrations-tab">
          ...
          <YTDSetting v-model="discordPresenceEnabled" type="checkbox" name="Discord rich presence" @change="settingsChanged" />
        ...
        <div v-if="currentTab === 99" class="about-tab">
          ...
          <h2 class="app-name">YouTube Desktop App</h2>
		  <p class="made-by">Made by aki</p>
		  <p class="made-by">From the contributions of YTMDesktop Team.</p>
          ...
          <span class="version-info">
            <p class="version">Version: {{ ytdVersion }}</p>
            <p class="branch">Branch: {{ ytdBranch }}</p>
            <p class="commit">Commit: {{ ytdCommitHash }}</p>
          </span>
        </div>
      </div>
    </div>
