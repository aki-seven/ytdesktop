...
function denyCompanion() {
  window.ytd.sendResult(false);
}

function allowCompanion() {
  window.ytd.sendResult(true);
}

onBeforeMount(async () => {
  appName.value = await window.ytd.getAppName();
  code.value = await window.ytd.getCode();
});
</script>
...
<template>
  <div class="container-wrapper">
    <div class="container">
      ...
      <p class="subtitle">
        <b>{{ appName }}</b> would like to control YouTube Desktop App
      </p>
