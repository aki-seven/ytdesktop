...
const minimizeWindow = window.ytd.minimizeWindow;
const maximizeWindow = window.ytd.maximizeWindow;
const restoreWindow = window.ytd.restoreWindow;
const closeWindow = window.ytd.closeWindow;

const openSettingsWindow = window.ytd.openSettingsWindow;
const navigateToDefault = window.ytd.ytViewNavigateDefault;

const wcoVisible = ref(window.navigator.windowControlsOverlay.visible);
const windowMaximized = ref(false);
const windowFullscreen = ref(false);

window.ytd.handleWindowEvents((event, state) => {
...
const ytViewUnresponsive = ref<boolean>(false);
...
if (props.isMainWindow) {
  ...
  onBeforeMount(async () => {
    ytViewUnresponsive.value = (await memoryStore.get("ytViewUnresponsive")) ?? false;
    appUpdateDownloaded.value = (await memoryStore.get("appUpdateDownloaded")) ?? false;
  });

  memoryStore.onStateChanged(newState => {
    ytViewUnresponsive.value = newState.ytViewUnresponsive;
  ...
  <div v-if="!windowFullscreen" class="titlebar">
    <div class="left">
      <div class="title">
        ...
        <p v-if="title && !centerTitleText" class="title-text">{{ title }}{{ ytViewUnresponsive ? " (Unresponsive)" : "" }}</p>
      </div>
    </div>
    <div v-if="title && centerTitleText" class="center">
      <p class="title-text">{{ title }}{{ ytViewUnresponsive ? " (Unresponsive)" : "" }}</p>
    </div>
    <div class="right">
