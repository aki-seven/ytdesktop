...
import { initializeAdblocker, injectYouTubeAdHiding, injectYouTubeAutoSkip, setAdBlockingEnabled } from "../core/adblock/blocker";
...
declare const YTD_DISABLE_UPDATES: boolean;
declare const YTD_UPDATE_FEED_OWNER: string;
declare const YTD_UPDATE_FEED_REPOSITORY: string;
...
log.errorHandler.startCatching({
  ...
  onError({ error, processType, versions }) {
    ...
    if (!app.isReady()) {
      dialog.showErrorBox(`YouTube Desktop App Crashed`, `Application crashed before ready\n\n${dialogMessage}`);
    } else {
      ...
      result = dialog.showMessageBoxSync({
        title: "Error",
        message: "YouTube Desktop App Crashed",
      ...
      if (result === 0 || result === 2) {
        clipboard.writeText(`YouTube Desktop App Crashed\n\n${dialogMessage}`);
      }
    }
...
const template: MenuItemConstructorOptions[] = [{ role: "appMenu", label: "YouTube Desktop App" }, { role: "editMenu" }];
...
const ytViewIntegrationScripts: { [name: string]: { [name: string]: string } } = {};

let mainWindow: BrowserWindow = null;
let settingsWindow: BrowserWindow = null;
let ytView: BrowserView = null;
let tray: Tray = null;
...
let ytViewLoadTimeout: NodeJS.Timeout | null = null;
...
if (!app.isDefaultProtocolClient("ytd")) {
  if (process.defaultApp) {
    if (process.argv.length >= 2) {
      log.info("Application set as default protcol client for 'ytd'");
      app.setAsDefaultProtocolClient("ytd", process.execPath, [path.resolve(process.argv[1])]);
    }
  } else {
    log.info("Application set as default protcol client for 'ytd'");
    app.setAsDefaultProtocolClient("ytd", process.execPath);
  }
}
...
memoryStore.onStateChanged((newState, oldState) => {
  ...
  if (ytView !== null) {
    ytView.webContents.send("memoryStore:stateChanged", newState, oldState);
  }
});
...
if (app.isPackaged && !shouldDisableUpdates() && !YTD_DISABLE_UPDATES) {
  const updateServer = "https://update.electronjs.org";
  const updateFeed = `${updateServer}/${YTD_UPDATE_FEED_OWNER}/${YTD_UPDATE_FEED_REPOSITORY}/${process.platform}-${process.arch}/${app.getVersion()}`;

  autoUpdater.setFeedURL({
    url: updateFeed
  });
  autoUpdater.on("checking-for-update", () => {
    if (appLaunchUpdateCheck) memoryStore.set("ytViewLoadingStatus", "Checking for updates...");
  ...
  autoUpdater.on("update-available", () => {
    ...
    if (appLaunchUpdateCheck) memoryStore.set("ytViewLoadingStatus", "Downloading update...");
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (ytView !== null) {
    ytView.webContents.send("settings:stateChanged", newState, oldState);
  }
  ...
  if (newState.appearance.zoom !== oldState.appearance.zoom) {
    if (ytView) {
      ytView.webContents.setZoomFactor(newState.appearance.zoom / 100);
  ...
  if (newState.appearance.customCSSEnabled) {
    customCss.provide(store, ytView);
  }
  ...
  // Ad blocking
  if (newState.adBlocking.enabled !== oldState.adBlocking.enabled) {
    if (newState.adBlocking.enabled) {
      log.info("Ad blocking enabled");
      setAdBlockingEnabled(true);
    } else {
      log.info("Ad blocking disabled");
  ...
  if (newState.integrations.companionServerEnabled) {
    companionServer.provide(store, memoryStore, ytView);
  }
...
// Functions with call to ytView renderer
function ytViewNavigated() {
  if (ytView !== null) {
    const url = ytView.webContents.getURL();
    if (url.startsWith("https://www.youtube.com/")) {
      lastUrl = url;
      ytView.webContents.send("ytView:navigationStateChanged", {
        canGoBack: ytView.webContents.navigationHistory.canGoBack(),
        canGoForward: ytView.webContents.navigationHistory.canGoForward()
      });
    }
  }
}
...
// Handles any navigation or window opening from ytView
function openExternalFromYtView(urlString: string) {
  ...
  } else if (domain === "youtube.com" && ytView) {
    ytView.webContents.loadURL(urlString);
  }
}
...
const createYTView = (): void => {
  memoryStore.set("ytViewLoadTimedout", false);
  memoryStore.set("ytViewLoading", true);
  memoryStore.set("ytViewLoadingStatus", "Initializing...");

  ytView = new BrowserView({
    webPreferences: {
      sandbox: true,
      contextIsolation: true,
      partition: app.isPackaged ? "persist:ytview" : "persist:ytview-dev",
      preload: path.join(__dirname, `../renderer/windows/ytview/preload.js`),
  ...
  // Attach events to yt view
  ytView.webContents.on("will-navigate", event => {
    const url = new URL(event.url);
    if (isPreventedNavOrRedirect(url)) {
      event.preventDefault();
      log.info(`Blocking ytView navigation to ${event.url}`);

      openExternalFromYtView(event.url);
    }
  });
  ytView.webContents.on("will-redirect", event => {
    const url = new URL(event.url);
    if (isPreventedNavOrRedirect(url)) {
      event.preventDefault();
      log.info(`Blocking ytView redirect to ${event.url}`);
    }
    ...
    if (
      ...
      ytView.webContents.loadURL(
        "https://accounts.google.com/ServiceLogin?service=youtube&continue=https%3A%2F%2Fwww.youtube.com%2Fsignin%3Faction_handle_signin%3Dtrue%26app%3Ddesktop%26next%3Dhttps%253A%252F%252Fwww.youtube.com%252F"
      );
    }
  });
  ytView.webContents.on("did-navigate", ytViewNavigated);
  ytView.webContents.on("did-navigate-in-page", ytViewNavigated);
  ytView.webContents.on("enter-html-full-screen", () => {
    if (mainWindow) {
      mainWindow.setFullScreen(true);
    }
  });
  ytView.webContents.on("leave-html-full-screen", () => {
    if (mainWindow) {
      mainWindow.setFullScreen(false);
    }
  });
  ytView.webContents.on("render-process-gone", () => {
    store.set("state.lastUrl", lastUrl);
    store.set("state.lastVideoId", lastVideoId);
    store.set("state.lastPlaylistId", lastPlaylistId);
    createYTView();
  });
  ytView.webContents.on("page-title-updated", (_event, title) => {
    if (mainWindow) {
      mainWindow.setTitle(`${title} | YouTube Desktop App`);
    }
  });
  ytView.webContents.on("context-menu", (_event, params) => {
    if (store.get("developer.enableDevTools")) {
      Menu.buildFromTemplate([
        ...
        },
        {
          ...
          click: () => {
            if (ytView) {
              ytView.webContents.openDevTools({
                mode: "detach"
              });
            }
          }
  ...
  ytView.webContents.on("will-prevent-unload", event => {
    if (mainWindow) {
      if (!applicationQuitting) {
        if (ytView.webContents.getURL().startsWith("https://www.youtube.com/")) {
          const choice = dialog.showMessageBoxSync(mainWindow, {
            type: "question",
            buttons: ["Leave", "Stay"],
            title: "Navigation",
            message: "YouTube is preventing navigation. Do you want to leave or stay?",
            defaultId: 0,
            cancelId: 1
          });
  ...
  ytView.webContents.on("unresponsive", () => {
    memoryStore.set("ytViewUnresponsive", true);
  });
  ytView.webContents.on("responsive", () => {
    memoryStore.set("ytViewUnresponsive", false);
  });

  ytView.webContents.setWindowOpenHandler(details => {
    openExternalFromYtView(details.url);
  ...
  ytView.webContents.on("did-start-loading", () => {
    memoryStore.set("ytViewLoadingStatus", "Loading YouTube...");
  });

  ytView.webContents.on("did-finish-load", async () => {
    if (!memoryStore.get("ytViewLoadingError")) {
      memoryStore.set("ytViewLoadingStatus", "Loaded YouTube");
    }
    const url = ytView.webContents.getURL();
    if (url.includes("youtube.com")) {
      await injectYouTubeAdHiding(ytView.webContents);
      await injectYouTubeAutoSkip(ytView.webContents);
    }
  });

  ytView.webContents.on("did-fail-load", (_event, errorCode, errorDescription, _validatedURL, isMainFrame) => {
    if (isMainFrame) {
      if (ytViewLoadTimeout) clearTimeout(ytViewLoadTimeout);

      memoryStore.set("ytViewLoadingError", true);
      memoryStore.set("ytViewLoadingStatus", `Failed to load YouTube: ${errorDescription} (${errorCode})`);
    }
  });

  memoryStore.set("ytViewLoadingStatus", "Initialized");
  ...
  if (continueWhereYouLeftOff) {
    ...
    if (lastUrl) {
      if (lastUrl.startsWith("https://www.youtube.com/")) {
        ytView.webContents.loadURL(lastUrl);
  ...
  if (navigateDefault) {
    ...
    store.set("state.lastUrl", "https://www.youtube.com/");
  }

  ytViewLoadTimeout = setTimeout(() => {
    memoryStore.set("ytViewLoadTimedout", true);
  }, 30 * 1000);
};
...
app.on("ready", async () => {
  ...
  try {
    ...
    await firstRunTouch.close();
      ...
      const v1ConfigPath = path.join(app.getPath("userData"), "..", "youtube-desktop-app", "config.json");
    try {
      ...
      const migrateDialog = await dialog.showMessageBox({
        ...
        detail:
          "A configuration file for YouTube Desktop App v1 was found. Your settings can be migrated.\n\nWARNING: Not all settings will be migrated as they may no longer be available in v2.",
  ...
  // Handle yt view ipc
  ipcMain.on("ytView:loaded", event => {
    if (ytView !== null && mainWindow !== null) {
      if (event.sender !== ytView.webContents) return;

      memoryStore.set("ytViewLoading", false);
      clearTimeout(ytViewLoadTimeout);
      ...
      ratioVolume.ytViewLoaded();
      // TODO: this is just a hack fix for custom css to update CSS when the view loads
      customCss.updateCSS();
    }
  });

  ipcMain.on("ytView:videoProgressChanged", (event, progress) => {
    if (event.sender !== ytView.webContents) return;

    playerStateStore.updateVideoProgress(progress);
  });

  ipcMain.on("ytView:videoStateChanged", (event, state) => {
    if (event.sender !== ytView.webContents) return;

    // yt state mapping definitions
    // -1 -> Unstarted
    // 1 -> Playing
    // 2 -> Paused
    // 3 -> Buffering
  ...
  ipcMain.on("ytView:videoDataChanged", (event, videoDetails, playlistId, album, likeStatus, hasFullMetadata) => {
    if (event.sender !== ytView.webContents) return;
  ...
  ipcMain.on("ytView:storeStateChanged", (event, queue, likeStatus, volume, muted, adPlaying) => {
    if (event.sender !== ytView.webContents) return;

    playerStateStore.updateFromStore(queue, likeStatus, volume, muted, adPlaying);
  });

  ipcMain.on("ytView:switchFocus", (event, context) => {
    if (event.sender !== ytView.webContents && event.sender !== mainWindow.webContents) return;

    if (context === "main") {
      if (mainWindow && ytView.webContents.isFocused()) {
        mainWindow.webContents.focus();
      }
    } else if (context === "yt") {
      if (ytView && mainWindow.webContents.isFocused()) {
        ytView.webContents.focus();
      }
    }
  });

  ipcMain.on("ytView:navigateDefault", event => {
    if (ytView) {
      if (event.sender !== mainWindow.webContents) return;
  ...
  ipcMain.on("ytView:recreate", event => {
    if (event.sender !== mainWindow.webContents) return;

    if (ytView) {
      if (mainWindow) {
        mainWindow.removeBrowserView(ytView);
      }

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (ytView.webContents as any).destroy();
      ytView = null;
      createYTView();
    }
  });

  ipcMain.handle("ytView:getIntegrationScripts", event => {
    if (event.sender !== ytView.webContents) return;

    return ytViewIntegrationScripts;
  });
  ...
  ipcMain.handle("settings:get", (event, key: string) => {
    if (
      ...
      ytView &&
      event.sender !== ytView.webContents
    )
      return;
  ...
  session.fromPartition(app.isPackaged ? "persist:ytview" : "persist:ytview-dev").setPermissionCheckHandler((webContents, permission) => {
    if (webContents == ytView.webContents) {
  ...
  session.fromPartition(app.isPackaged ? "persist:ytview" : "persist:ytview-dev").setPermissionRequestHandler((webContents, permission, callback) => {
    if (webContents == ytView.webContents) {
      if (permission === "fullscreen") {
  ...
  memoryStore.set("ytViewLoading", true);
  memoryStore.set("ytViewLoadingStatus", "Checking for updates...");
  ...
  ytViewIntegrationScripts["ratioVolume"] = ratioVolume.getYTScripts().reduce<{ [name: string]: string }>((map, obj) => {
  ...
  if (store.get("appearance").zoom) {
    log.info("Integration update: Zoom Factor");
    ytView.webContents.setZoomFactor(store.get("appearance").zoom / 100);
  }
