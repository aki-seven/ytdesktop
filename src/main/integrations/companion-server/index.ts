...
export default class CompanionServer implements IIntegration {
  ...
  private createServer() {
    ...
    this.fastifyServer.setErrorHandler((error: unknown, request, reply) => {
      const fastifyError = error as FastifyError;
      if (!isDefinedAPIError(fastifyError)) {
        if (!fastifyError.statusCode || fastifyError.statusCode >= 500) {
          log.error(fastifyError);
  ...
  public provide(store: Conf<StoreSchema>, memoryStore: MemoryStore<MemoryStoreSchema>, ytView: BrowserView): void {
    this.store = store;
