  ...
  return {
    ...
    video: state.videoDetails
      ? {
          ...
          // API Users: The initial player response for a song may not contain filled out metadata is fetched via another requests. This indicates that is complete.
          //            For context you can refer to YouTube playing a song but the player bar not displaying the information for the song yet. This is that behavior.
    ...
    // - If the song playing comes from a randomly generated radio queue then this will be the id of that random queue (YouTube does not persist these, pretend these IDs don't exist on the YouTube backend)
...
const CompanionServerAPIv1: FastifyPluginCallback<CompanionServerAPIv1Options> = async (fastify, options) => {
  const sendCommand = (commandRequest: APIV1CommandRequestBodyType) => {
    const ytView = options.getYtView();
    if (ytView) {
      switch (commandRequest.command) {
        case "playPause": {
          ytView.webContents.send("remoteControl:execute", "playPause");
          break;
        }

        case "play": {
          ytView.webContents.send("remoteControl:execute", "play");
          break;
        }

        case "pause": {
          ytView.webContents.send("remoteControl:execute", "pause");
          break;
        }

        case "volumeUp": {
          ytView.webContents.send("remoteControl:execute", "volumeUp");
          break;
        }

        case "volumeDown": {
          ytView.webContents.send("remoteControl:execute", "volumeDown");
          break;
        }
        ...
        case "setVolume": {
          ...
          ytView.webContents.send("remoteControl:execute", "setVolume", volume);
          break;
        }

        case "mute": {
          ytView.webContents.send("remoteControl:execute", "mute");
          break;
        }

        case "unmute": {
          ytView.webContents.send("remoteControl:execute", "unmute");
          break;
        }
        ...
        case "seekTo": {
          ...
          ytView.webContents.send("remoteControl:execute", "seekTo", position);
          break;
        }
        ...
        case "changeVideo": {
          ...
          ytView.webContents.send("remoteControl:execute", "navigate", {
            watchEndpoint: {
              videoId: videoId,
        ...
        case "next": {
          ytView.webContents.send("remoteControl:execute", "next");
          break;
        }

        case "previous": {
          ytView.webContents.send("remoteControl:execute", "previous");
          break;
        }
        ...
        case "repeatMode": {
          ...
          switch (repeatMode) {
            case RepeatMode.None: {
              ytView.webContents.send("remoteControl:execute", "repeatMode", "NONE");
              break;
            }
            case RepeatMode.All: {
              ytView.webContents.send("remoteControl:execute", "repeatMode", "ALL");
              break;
            }
            case RepeatMode.One: {
              ytView.webContents.send("remoteControl:execute", "repeatMode", "ONE");
              break;
            }
            default: {
        ...
        case "shuffle": {
          ytView.webContents.send("remoteControl:execute", "shuffle");
          break;
        }
        ...
        case "playQueueIndex": {
          ...
          ytView.webContents.send("remoteControl:execute", "playQueueIndex", index);
          break;
        }

        case "toggleLike": {
          ytView.webContents.send("remoteControl:execute", "toggleLike");
          break;
        }

        case "toggleDislike": {
          ytView.webContents.send("remoteControl:execute", "toggleDislike");
          break;
        }
      }
    }
  ...
  fastify.get(
    ...
    {
      config: {
        // This endpoint sends a real API request to YouTube which allows to fetch playlists.
    ...
    async (request, response) => {
      ...
      if (ytView) {
        ...
        const playlistsResponseListener = (event: Electron.IpcMainEvent, playlists: Playlist[]) => {
          if (event.sender !== ytView.webContents) return;
          response.send(playlists);
        };
        ipcMain.once(`ytView:getPlaylists:response:${requestId}`, playlistsResponseListener);

        ytView.webContents.send(`ytView:getPlaylists`, requestId);

        await new Promise((_resolve, reject) =>
          setTimeout(() => {
            ipcMain.removeListener(`ytView:getPlaylists:response:${requestId}`, playlistsResponseListener);
            reject(new YouTubeTimeOutError());
          }, 1000 * 30)
        );
      } else {
  ...
  fastify.ready().then(() => {
    ...
    ipcMain.on("ytView:createPlaylistObserved", createPlaylistObservedListener);
    ...
    ipcMain.on("ytView:deletePlaylistObserved", deletePlaylistObservedListener);
    ...
    fastify.addHook("onClose", () => {
      ...
      ipcMain.off("ytView:createPlaylistObserved", createPlaylistObservedListener);
      ipcMain.off("ytView:deletePlaylistObserved", deletePlaylistObservedListener);
    });
  });
};
